...
defmodule PlotRegistry do
  ...
  def register(state, owner),
    do:
      with(
        plot <- %Plot{plot_id: state.__next_id__, registered_to: owner},
        state <- Map.put_new(state, state.__next_id__, plot),
        state <- %{state | :__next_id__ => state.__next_id__ + 1},
        do: {plot, state}
      )

  @spec release(map(), integer()) :: map()
  def release(state, plot_id),
    do: Map.drop(state, [plot_id])

  @spec all(map()) :: [Plot]
  def all(state), do: Map.values(state) |> Enum.filter(&is_struct(&1, Plot))

  @spec get(map(), integer()) :: Plot
  def get(state, plot_id) do
    case Map.fetch(state, plot_id) do
      {:ok, plot} -> plot
      :error -> {:not_found, "plot is unregistered"}
    end
  end
end
