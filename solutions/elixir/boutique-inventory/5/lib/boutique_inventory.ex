defmodule BoutiqueInventory do
  import Map, only: [update: 4, values: 1]
  ...
  def increase_quantity(item, amount),
    do: item |> update(:quantity_by_size, %{}, &increase_quantities(&1, amount))

  @spec increase_quantities(map(), integer()) :: map()
  defp increase_quantities(quantity_by_size, amount),
    do: for({size, quantity} <- quantity_by_size, into: %{}, do: {size, quantity + amount})
