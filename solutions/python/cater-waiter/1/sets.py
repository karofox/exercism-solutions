...
def categorize_dish(dish_name: str, dish_ingredients: list[str]) -> str:
    ...
    """
    match ingeredients := {*dish_ingredients}:
        case _ if not ingeredients - VEGAN:
            return f"{dish_name}: VEGAN"
        case _ if not ingeredients - VEGETARIAN:
            return f"{dish_name}: VEGETARIAN"
        case _ if not ingeredients - PALEO:
            return f"{dish_name}: PALEO"
        case _ if not ingeredients - KETO:
            return f"{dish_name}: KETO"
        case _ if not ingeredients - OMNIVORE:
