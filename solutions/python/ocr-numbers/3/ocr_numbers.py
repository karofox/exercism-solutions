...
def convert(input_grid: list[str]) -> str:
    ...
    if len(input_grid) % ROW_LEN != 0:
        raise ValueError("Number of input lines is not a multiple of four")
    if len(input_grid[0]) % COL_LEN != 0:
...
def segmentate_lines(line_grid: list[str]) -> list[tuple[str, ...]]:
    ...
    return [
        tuple(line[start : start + COL_LEN] for line in line_grid)
...
def segmentate_rows(grid: list[str]) -> list[list[str]]:
    ...
    return [grid[start : start + ROW_LEN] for start in range(0, len(grid), 4)]
