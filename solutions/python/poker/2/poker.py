...
class Hand:
    def __init__(self, data: str):
        self.cards = sorted([Card(card) for card in data.split()])
        self.by_values = [card.value for card in self.cards]
        self.data = data
    ...
    def value_counts(self) -> dict[int, int]:
        ...
        for value, count in collections.Counter(self.by_values).items():
    ...
    def _is_straight(self) -> bool:
        if 14 in self.by_values:
            return (self.by_values == [2, 3, 4, 5, 14] 
                    or self.by_values == [10, 11, 12, 13, 14])
        first_value = self.by_values[0]
        return self.by_values == [
            first_value,
            first_value + 1,
            first_value + 2,
            first_value + 3,
    ...
    def _check_low_straight(self) -> None:
        if self.by_values == [2, 3, 4, 5, 14]:
            self.by_values = [1, 2, 3, 4, 5]
