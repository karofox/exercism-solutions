...
def value_of_card(card: str) -> int:
    ...
    Raises ValueError if provided string is not a card.
    """
    match card:
        case "J" | "Q" | "K":
            return 10
        case "A":
            return 1
        case _ if 2 <= (numerical_value := int(card)) <= 10:
...
def higher_card(card_one: str, card_two: str) -> str | tuple[str, str]:
    ...
    return card_one if card_one > card_two else card_two
...
def is_blackjack(card_one: str, card_two: str) -> bool:
    ...
    match card_one, card_two:
        case ("A", card) | (card, "A") if value_of_card(card) == 10:
            return True
        case _:
            return False
